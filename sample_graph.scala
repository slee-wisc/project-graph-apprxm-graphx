...
object GraphSample {
	...
	def main(args: Array[String]): Unit = {
		if (args.length < 4) {
  		System.err.println(
    		"Usage: ProjectApp <method> <ratio> <infile> <outfile>")
  		System.exit(1)
  	}
    ...
    GraphXUtils.registerKryoClasses(conf)
		...
		val sc = new SparkContext(conf.setAppName("Sampling(" + fname + method + ")"))
    val graph = GraphLoader.edgeListFile(sc, fname)
    val subgraph = sampling(graph,method,ratio)

    val writer = new PrintWriter(new File(outname))
    val idset = subgraph.vertices.map{case (id,att) => id}.collect()
    val newedge = graph.edges.filter{e=> idset.contains(e.srcId) && idset.contains(e.dstId)}
    newedge.collect().foreach(e => writer.write(e.srcId + " " + e.dstId+"\n"))
    //writer.write()
    writer.close()
	  //subgraph.saveAsTextFile(outname)
	}
}
